...
class Eyes:
    ...
    def __init__(self, cross_eye=0, default_look_distance=200):
        self.default_look_distance = default_look_distance
        self.look_target = Vector((0, self.default_look_distance, 0))
        ...
        self.pupil_bounds = [[-20, 20], [-15, 15], [-20, 20], [-15, 15]]
    ...
    def update_look_target(self, forced=False):
        if forced or sum(
                [self.pupil_bounds[i][0] < p < self.pupil_bounds[i][1] for i, p in enumerate(self.pupil_pos)]) != 4:
            new_look_target = Vector((0, self.default_look_distance, 0))
